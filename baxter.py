...
class BaxterRobot:
    ...
    def __init__(self, arm, camera="left_hand_camera", rate=100):
        ...
        #self._cam_image_sub = rospy.Subscriber("/cameras/"+camera+"/image", Image, self._fill_image_data)
    ...
    def get_cartesian_position(self, position, orientation):
        hdr = Header(stamp=rospy.Time.now(), frame_id="base")
        msg = PoseStamped(
            header=hdr,
            pose=Pose(
